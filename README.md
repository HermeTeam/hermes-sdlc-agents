...
Repositories are not mounted into agent containers. In the MVP, Hermes agents connect directly to a role-scoped GitHub/GitLab API/MCP endpoint through `GIT_PROVIDER_MCP_URL`. Hermes sees only narrow allowlisted repository tools: read/tree/search, task branch, patch/commit, change request, review comments, and CI evidence.

For the builder this means:

- no `REPO_DIR` and no `/workspace/repo`;
- no broad GitHub/GitLab token in the agent container; only a short-lived role-scoped provider MCP token is used;
...
- GitHub Pull Requests and GitLab Merge Requests are represented in the workflow as `change_request`;
...
Requirements: Docker Engine with Compose v2, an OpenAI-compatible LLM gateway, and a Streamable HTTP GitHub/GitLab API/MCP endpoint that follows `docs/GIT_PROVIDER_INTEGRATION.md`. Compose is intended for local/single-host runs; for production, restrict egress with a firewall/egress proxy or use the Kubernetes NetworkPolicy from this bundle.
...
2. Keep the `test-project/test-project` GitHub repository target in `.env.example` and set `GIT_PROVIDER_MCP_URL` for the direct GitHub/GitLab API/MCP endpoint.
3. Replace all `CHANGE_ME` values in every `secrets/hermes-*.env` file.
4. Issue six different `GIT_PROVIDER_MCP_TOKEN` values; one token must not be reused across roles.
...
The MVP does not use a separate repository gateway. Hermes roles connect directly to GitHub/GitLab API/MCP, while the rest of the control plane remains separate narrow integrations:

- GitHub or GitLab for repository/change request/review and branch protection through the direct provider API/MCP endpoint;
...
Do not give Hermes a broad GitHub/GitLab token, Kubernetes kubeconfig, Argo admin token, cloud credential, or runner shell. In the MVP, only a short-lived role-scoped provider token limited to allowlisted repository operations is acceptable.
